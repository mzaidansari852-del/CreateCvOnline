...
| **Payments** | PayPal orders, server-side capture with amount and currency verification, signed webhooks, idempotent fulfilment. |
...
| `PAYPAL_CLIENT_ID` / `PAYPAL_CLIENT_SECRET` / `PAYPAL_ENVIRONMENT` | Payments | `sandbox` or `live`. |
...
5. Update the PayPal webhook URL to the production domain.
