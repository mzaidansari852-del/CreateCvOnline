...
import { gatewayFor, paypalCaptureMatchesPlan, readPayPalCustomId } from '@/lib/payments';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const verified = await gatewayFor('paypal').verifyWebhook(headers, rawBody);
  if (!verified) {
  ...
  const identity = readPayPalCustomId(resource.custom_id);
  ...
  try {
    switch (type) {
      ...
      case 'PAYMENT.CAPTURE.COMPLETED': {
        ...
        const capture = await gatewayFor('paypal').getOrder(orderId);
        if (capture.status === 'completed' && paypalCaptureMatchesPlan(capture, planId)) {
          await fulfilPayment({ userId: identity.userId, planId, capture, provider: 'paypal' });
        } else {
