...
export const meta: TemplateMeta = {
  ...
  tagline:
    "Compact one-page rhythm that treats projects and volunteering as real experience.",
  description:
 ...
 */
export default function AtsEntryLevel({
  cv,
  customization: c,
}: CVTemplateProps) {
  ...
  const tracking =
    c.headingCase === "uppercase" ? "0.18em" : headingTracking(c);
  const name = fullName(cv);

  return (
    <div style={{ padding: c.pageMargin }}>
      <header
        style={{ paddingBottom: "0.45em", borderBottom: `2px solid ${accent}` }}
      >
        <div
          style={{
            display: "flex",
        ...
        >
          ...
          {cv.personal.title ? (
            <p style={{ fontSize: "1em", color: metaInk, fontWeight: 600 }}>
              {cv.personal.title}
            </p>
          ) : null}
        </div>
