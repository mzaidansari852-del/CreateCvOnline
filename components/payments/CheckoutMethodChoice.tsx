 ...
 * customer nothing and still asks them to read it. While PayPal is the interim gateway and
 * Polar is not live, that is the normal case; this exists for the overlap.
 *
 ...
 *
 * The gateway names are deliberately absent from the tiles. "Polar" is a company the
 ...
 * reassures rather than confuses. PayPal is the exception, because there the brand *is*
 * the payment method the customer is choosing.
 *
 * The default comes from `availableGateways()`'s preference order rather than being fixed
 * here, so the deployment decides. Today that means Polar when it is configured: it is the
 * merchant of record, which is the difference between charging the right tax and owing it.
 */

export type CheckoutMethod = 'polar' | 'paypal';
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-2.5">
        ...
        >
          <MethodTile
            selected={method === 'polar'}
            onSelect={() => setMethod('polar')}
            name={copy.checkout.methodCard}
            hint={copy.checkout.methodCardHint}
            art={<CardBrandMarks />}
          />
          <MethodTile
      ...
      {/*
        Both buttons stay mounted rather than being swapped, because either can be holding
        a failure and the retry that goes with it. Throwing that away on an idle click of
        the other tile would leave paying a second time as the customer's only visible way
        forward.
      */}
      <div hidden={method !== 'polar'}>
        <PolarCheckoutButton planId={planId} planName={planName} priceLabel={priceLabel} />
      </div>
      <div hidden={method !== 'paypal'}>
        <PayPalCheckoutButton planId={planId} planName={planName} priceLabel={priceLabel} />
      </div>
    </div>
  );
}
