 ...
 *
 * The interim counterpart to `PolarCheckoutButton`. Both do the same two things — ask our
 * server for a checkout and navigate to it — which is why the checkout page can mount
 * either one without knowing anything about the gateway behind it.
 *
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // The code, not the API's English `message`: the API's is written for whoever
        // reads the logs, this is read by whoever is holding the card, under a translated
        // heading.
        setError(
